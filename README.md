...
- Restart radia podłączonego przez USB (port szeregowy zostaje otwarty, a radio po starcie milczy, dopóki klient nie poprosi o konfigurację) jest wykrywany po komunikatach startowych na porcie albo po kilku minutach bez pakietów — integracja sama odnawia wtedy sesję z radiem
- Kanał bez własnej nazwy nazywa się w panelu, ustawieniach i encjach tak jak w firmware i aplikacji — nazwą presetu LoRa (np. „MediumFast”)
...
- A restart of a radio connected over USB (the serial port stays open and the radio stays silent after boot until the client asks for its configuration) is detected from the boot messages on the port or after a few minutes without packets — the integration then renews the session with the radio by itself
- A channel without its own name is named in the panel, settings and entities the same way as in the firmware and the app — after the LoRa preset (e.g. "MediumFast")
