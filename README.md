...
- Sniffer (Settings → Sniffer): enabled and disabled through OnDemand, a warning before enabling, a live log of captured packets (up to 5000 in memory) with a filter, pause and export to JSON and CSV. The sniffer state lives in the radio's RAM, so it is always off after a reboot, and it can only be enabled on the gateway connected to Home Assistant
