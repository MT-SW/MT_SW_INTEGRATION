...
- Messages: channels and direct messages, sending, history stored in Home Assistant, delivery status (pending, sent to the mesh, acknowledged by the recipient). In the bubble of a received message a direct link shows SNR and RSSI, and a relayed one shows the hop count and the relay as "via (short name)". The message details show the route, the relay it came through, the signal level (SNR and RSSI), the XEdDSA signature and, for sent messages, the relays that heard it
