...
- Węzły: lista z filtrami (ulubione, bezpośrednie, z pozycją, śledzone, bez MQTT, ignorowane), szczegóły z telemetrią i pozycją oraz akcje: wiadomość, ulubiony i ignorowanie, pobranie pozycji, pobranie sąsiadów, traceroute, usunięcie węzła. Historia tras, sąsiadów, pozycji, telemetrii, pakietów i zasobów pokazuje się na żądanie, tylko dla otwartego węzła
...
- Ustawienia: pełna konfiguracja radia podłączonego do Home Assistanta — LoRa, kanały (z kluczem PSK), użytkownik, urządzenie, pozycja, zasilanie, sieć, wyświetlacz, Bluetooth, bezpieczeństwo i ekran. Moduły: MQTT (z raportowaniem map), port szeregowy, powiadomienia zewnętrzne, Store & Forward, test zasięgu, telemetria, wiadomości predefiniowane, audio, informacje o sąsiadach, oświetlenie, czujnik detekcji, Paxcounter oraz moduły MT_SW: zarządzanie ruchem, Meshbeacon i wiadomość statusu. Ponadto Sniffer, akcje urządzenia (restart, wyłączenie, resety) i zarządzanie danymi zapisanymi przez panel
...
- Nodes: a list with filters (favorites, direct, with position, tracked, hide MQTT, ignored), details with telemetry and position, and actions: message, favorite and ignore, request position, request neighbors, traceroute, remove node. Histories of routes, neighbors, position, telemetry, packets and resources are shown on demand, only for the open node
...
- Settings: full configuration of the radio connected to Home Assistant — LoRa, channels (with PSK), user, device, position, power, network, display, Bluetooth, security and screen. Modules: MQTT (with map reporting), serial, external notification, Store & Forward, range test, telemetry, canned messages, audio, neighbor info, ambient lighting, detection sensor, Paxcounter, plus MT_SW modules: traffic management, Meshbeacon and status message. Also the Sniffer, device actions (reboot, shutdown, resets) and management of the data stored by the panel
