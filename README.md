...
- Wiadomości: kanały i wiadomości bezpośrednie, wysyłanie, historia zapisywana w Home Assistancie, status doręczenia (oczekuje, wysłano do sieci, potwierdzona przez odbiorcę). Przy każdej odebranej wiadomości widać sygnał, z jakim dotarła (SNR i RSSI), liczbę skoków i przekaźnik w postaci „via (krótka nazwa)”. Szczegóły wiadomości pokazują trasę, przekaźniki, które ją usłyszały, i podpis XEdDSA
...
Funkcje firmware MT_SW (wymagają radia z firmware MT_SW, które zgłasza FW+ w wersji 2 lub nowszej; od tej wersji są dostępne wszystkie naraz, a panel pokazuje je tylko wtedy):
...
- Messages: channels and direct messages, sending, history stored in Home Assistant, delivery status (pending, sent to the mesh, acknowledged by the recipient). Every received message shows the signal it arrived with (SNR and RSSI), the hop count and the relay as "via (short name)". The message details show the route, the relays that heard it and the XEdDSA signature
