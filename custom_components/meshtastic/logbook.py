...
from .helpers import device_by_identifier
from .entity import (
...
async def async_setup_message_logger(hass: HomeAssistant, entry: MeshtasticConfigEntry) -> CALLBACK_TYPE:  # noqa: PLR0915
    ...
    async def _on_text_message(
        ...
        from_device = device_by_identifier(device_registry, (DOMAIN, str(from_node_id)), config_entry_id)
    ...
    def extract_device_and_entity_from_channel(
        ...
        if (to_channel_id := to.get("channel", None)) is not None:
            ...
            to_device = device_by_identifier(device_registry, (DOMAIN, str(gateway_node_id)), config_entry_id)
        else:
    ...
    def extract_device_and_entity_from_node(
        ...
        if (to_node_id := to.get("node", None)) is not None:
            to_device = device_by_identifier(device_registry, (DOMAIN, str(to_node_id)), config_entry_id)
